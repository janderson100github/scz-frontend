...
class PoolEditModal extends React.Component {
  ...
  get(name) {
    API.get('/pool/' + name, {}).then((res) => {
      this.setState({ modalIsOpen: false });
      let url = '/credit/edit/' + this.state.name + '?editId=' + this.state.editId;
        ...
        >
          <form onSubmit={event => this.handleSubmitForm(event)}>
            <div className="form-group">
              <input className="form-control" type="text" defaultValue={this.state.name}
                     placeholder="Name" name="name" onChange={event => this.handleChange(event)}
                     onKeyPress={event => {
                       if (event.key === "Enter") {
